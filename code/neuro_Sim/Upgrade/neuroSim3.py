...
minRate = 15           # maximum average spike-rate for poisson neurons
maxRate = 45           # maximum average spike-rate for poisson neurons
rate_step = 15
rateRange = maxRate - minRate
noiseRate = 100

#samples = []                           # object storing experimental sample set
stimuli = int(rateRange/rate_step) +1   # number of stimuli (strengths)
times   = 100                            # repetition times = minimum * constant
...
# to store spike trains elicited from each neuron over trials
  neuro_var = [ [] for i in range(IF+PS) ]
  stim_var  = [ [] for i in range(PS)    ]
  for si in range(stimuli) :
    for sj in range(stimuli) :
      rate_i = minRate + si*rate_step
      rate_j = minRate + sj*rate_step
...
#-USING-THE_BIALEK-METHOD-------------------------------------------------------
  MI_2_0_B_g = B_MI(stim_var[0], neuro_var[0], bdt, T, fsize, times)
...
#-USING-THE_BIALEK-METHOD-------------------------------------------------------
  MI_3_0_B_g = B_MI(stim_var[1], neuro_var[0], bdt, T, fsize, times)
...
plt.scatter(g_range, MI_3_0_B, marker = '+', color = 'black',
            label='$MI_{3,0}$ computed using the Bialek Method'+
            '\n$g_{3,0}=1-g,$ mean $\\mu='+str(round(MI_3_0_B_E,2))+
            '$\ncorrelation coefficient $\\rho='+
